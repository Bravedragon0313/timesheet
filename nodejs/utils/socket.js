...
class Socket {
    ...
    socketEvents() {
        this.io.on("connection", socket => {
            ...
            /**
             * Update read status of messages from 'from_id' to 'to_id'
             */
            socket.on("updateMessage", async updateData => {
                const result = await helper.updateMessage(
                    updateData.from_id,
                    updateData.to_id
                );                
            });
            ...
            socket.on("addMessage", async response => {
                ...
                socket
                    .to(response.toSocketId)
                    .emit("addMessageResponse", response);
            });
    ...
    socketConfig() {
        this.io.use(async (socket, next) => {
            ...
            if (response && response !== null) {
                ...
                console.error(
                    `Socket connection failed, for  user Id ${userId}.`
                );
            }
        });
        this.socketEvents();
